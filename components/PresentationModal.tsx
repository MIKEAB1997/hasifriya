...
const PresentationModal: React.FC<PresentationModalProps> = ({ presentation, onClose }) => {
  ...
  // פונקציית עזר להמרת לינק Drive לנגן מובנה
  const getEmbedLink = (url: string) => {
    if (!url || url === '#' || url.trim() === '') return null;
    if (url.includes('drive.google.com')) {
      let embedUrl = url;
      // החלפת view/edit ב-preview כדי לאפשר נגן מובנה עם דפדוף
  ...
  const embedUrl = getEmbedLink(presentation.driveUrl);
                ...
                {hasValidLink && embedUrl ? (
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center bg-surface/20 gap-6 p-12 text-center">
                     ...
                     <div className="space-y-2">
                        ...
                        <p className="text-gray-500 max-w-sm">לא הוגדר קישור Drive תקין. פנה למנהל המערכת לעדכון הקישור.</p>
                     </div>
                  </div>
                )}
             </div>
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              ...
              {hasValidLink && (
                <div className="flex flex-col sm:flex-row lg:flex-col gap-4 sm:w-auto lg:w-72">
                  ...
                  <a href={presentation.driveUrl.replace('/preview', '/view').replace('/edit', '/view')} download target="_blank" className="flex-1 flex items-center justify-center gap-3 bg-white/5 hover:bg-white/10 text-white font-bold py-5 px-8 rounded-2xl transition-all border border-white/10 active:scale-95">
